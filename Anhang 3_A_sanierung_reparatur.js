...
export function sanierung_reparatur(zustand, inspection, kante, BAA_AB, BAC_AC, BAD_CD, BAH_A,BAJ_BC, BBE_GH, BAF_I)
{
    let san_rep = {}  // sanierungsmassnahmen für Reparatur
    let lvz_flag_11 = true
    let lvz_flag_12 = true
    let lvz_flag_13 = true
    let lvz_flag_14 = true
    let lvz_flag_15 = true
    let lvz_flag_18 = true
    let lvz_flag_22 = true
    let mehrfachschaden = false
    ...
    const user = inspection.masnahmen.moegliche_masnahmen.find(user => user.name === "Reparatur")
    const reparatur = user.benoetigte_arbeiten

    // allgemeine Baustelleneinrichtung für jede Kante
    let mengen = [] //empty
    let von = get(zustand[0].station) // This is a programming trick, this value is always zero
            ...
            if( element.sanierung_schaden == "erforderlich")
            {
                ...
                if(element.streckenschaden?.value.includes("A") || get(element.streckenschaden) == null)
                {
                    ...
                    if(get(element.kode)=="BAA" && element.klassifizierung_dwa.schadensklasse_gesamt <= BAA_AB)
                    {
                        ...
                        {
                            san_rep={grund: KodeCh1, menge, station_von: von, station_bis: bis, laenge, klasse: 2 , lvz: "16.6", bauweise}
                            reparatur.push(san_rep) 
                            ...
                            for (let i=0; i<6; i++)
                            {
                                if(get(kante.mittlere_tiefe)<tiefe_teil[i])
                                {
                                    if(allow)
                                    {
                                        ...
                                        san_rep={grund: KodeCh1, menge, station_von: von, station_bis: bis, laenge, klasse, lvz: teilerneuerung_lvz[i], bauweise}
                                        reparatur.push(san_rep)
                                        allow=false
                                    }
                                }  
                    ...
                    // reparatur Kode: BAB
                    if(get(element.kode) == "BAB" && Number(get(kante.profil.profilbreite)) < 800)
                    {
                        lvz="14.1"
                        if(lvz_flag_14)
                        {
                            san_rep = {grund: KodeCh1, station_von: von, station_bis: bis, menge, laenge, klasse, lvz}
                            reparatur.push(san_rep)
                        }
                        lvz_flag_14=false
                        if(laenge>=laenge_kurzline[6])
                        {
                            san_rep={grund: KodeCh1, menge: Math.ceil(laenge/4), station_von: von, station_bis: bis, lage_umfang_von: umfang_von, lage_umfang_bis: umfang_bis, laenge, klasse, lvz: "14.9", bauweise}
                            reparatur.push(san_rep)    
                        }else
                        {
                            for (let i = 0; i<7; i++)
                            {
                                if(laenge<laenge_kurzline[i])
                                {
                                    if(allow)
                                    {
                                        san_rep={grund: KodeCh1, menge, station_von: von, station_bis: bis, lage_umfang_von: umfang_von, lage_umfang_bis: umfang_bis, laenge:laenge_kurzline[i], klasse, lvz: kurzliner_lvz[i], bauweise}
                                        reparatur.push(san_rep)
                                        allow=false
                                    }
                                }
                        ...
                        continue
                    } else if(get(element.kode) == "BAB" && Number(get(kante.profil.profilbreite)) >= 800)
                    {
                        lvz="13.1"
                        if(lvz_flag_13)
                        {
                        ...
                        san_rep = {grund: KodeCh1, menge, station_von: von, station_bis: bis, lage_umfang_von: umfang_von, lage_umfang_bis: umfang_bis, laenge, klasse, lvz, bauweise}
                        reparatur.push(san_rep)
                        continue    
                    }
                    ...
                    // reparatur Kode: BAF
                    if(get(element.kode)=="BAF" && (get(element.charakterisierung_1)=="F" || get(element.charakterisierung_1)=="H" || get(element.charakterisierung_1)=="G"))
                    {
                        if(get(element.streckenschaden) == null && Number(get(kante.profil.profilbreite))<800)
                        {
                            lvz = "11.1"
                    ...
                    if(get(element.kode)=="BAF" && (get(element.charakterisierung_1)=="J" || get(element.charakterisierung_1)=="K" || get(element.charakterisierung_1)=="Z"))
                    {
                        lvz="Ingenieurmaessige Begutachtung"
                        menge=""
                        bauweise=""
                    ...
                    }else if(get(element.kode)=="BAH" && (get(element.charakterisierung_1) =="B" || get(element.charakterisierung_1)=="C"))
                    {
                        if(kante.profil.profilbreite<800)
                        {
                            lvz = "12.1"
                            if(lvz_flag_12)
                            {
                            ...
                            lvz_flag_12=false
                            if(get(element.charakterisierung_1) =="B")
                            {
                                lvz = "12.5"
                            }else
                            {
                    ...
                    if(get(element.kode)=="BAI")
                    {
                        ...
                        if(Number(get(kante.profil.profilbreite))<800)
                        {
                            ...
                            lvz_flag_11=false
                            lvz = "11.6"
                            san_rep = {grund: KodeCh1, menge: laenge, station_von: von, station_bis: bis, lage_umfang_von: umfang_von, lage_umfang_bis: umfang_bis, laenge, klasse, lvz, bauweise}
                            reparatur.push(san_rep) 
                        }
                        else
                        {
                            ...
                            lvz_flag_18=false
                            lvz = "18.5"
                            san_rep = {grund: KodeCh1, menge: Umfang_Area, station_von: von, station_bis: bis, lage_umfang_von: umfang_von, lage_umfang_bis: umfang_bis, laenge, klasse, lvz, bauweise}
                            reparatur.push(san_rep)
                        }
                        continue
                    }
                    ...
                    {   if(get(element.charakterisierung_1)=="A")
                        {
                            if(Number(get(kante.profil.profilbreite))<800)
                            {
                                lvz = "11.1"
                        ...
                        }else if ((get(element.charakterisierung_1)=="C" || get(element.charakterisierung_1)=="B") && element.klassifizierung_dwa.schadensklasse_gesamt <= BAJ_BC)
                        {
                            bauweise="offen"
                            if(get(kante.mittlere_tiefe)>=tiefe_teil[5])
                                {
                            ...
                            if(Number(get(kante.profil.profilbreite))<800)
                            {
                                ...
                                }else
                                {
                                    if(laenge>=laenge_kurzline[6])
                                    {
                                        if(allow)
                                            {
                                                san_rep={grund: KodeCh1, menge, station_von: von, station_bis: bis, laenge, klasse, lvz: kurzliner_lvz[i], bauweise}
                                                reparatur.push(san_rep)
                                                allow=false
                                            }  
                                    }else
                    ...
                    if(get(element.kode)=="BBE" && check_true(BBE_CH1, get(element.charakterisierung_1)))
                    {
                        ...
                        reparatur.push(san_rep)
                        continue
                    }else if(get(element.kode)=="BBE" && check_true(BBE_CH1GH, get(element.charakterisierung_1)) && element.klassifizierung_dwa.schadensklasse_gesamt <= BBE_GH)
                    {
                        bauweise="offen"
                        if(get(kante.mittlere_tiefe)>=tiefe_teil[5]) 
                        {
                    ...
                    if(get(element.kode)=="BBF")
                    {
                        if(Number(get(kante.profil.profilbreite))<800)
                        {
                            ...
                            reparatur.push(san_rep)
                            menge=1
                            for(const runner of zustand)
                            {
                                if(get(runner.kode)!=get(element.kode) && get(runner.station) == get(element.station) && get(runner.streckenschaden) == null && runner.sanierung_schaden == "erforderlich")
                                {
                                    mehrfachschaden = true
                                }
                            }
                            if(mehrfachschaden == false)
                            {
                                lvz="14.1"
                                san_rep = {grund: KodeCh1, station_von: von, station_bis: bis, menge, laenge, klasse, lvz}
                                reparatur.push(san_rep)
